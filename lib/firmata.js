...
var SPI_DATA = 0x68;
var SPI_CONFIG = 0x00;
var SPI_BEGIN_TRANSACTION = 0x01;
var SPI_END_TRANSACTION = 0x02;
var SPI_TRANSFER = 0x03;
var SPI_REPLY = 0x04;
var SPI_END = 0x05;
...
SYSEX_RESPONSE[SPI_DATA] = function(board) {
  ...
  var numBytes = board.currentBuffer[4];
  var reply = [];
  console.log("reply: deviceId = " + deviceId);
  console.log("reply: numBytes = " + numBytes);
  // skip numBytes for now
  //reply.push(numBytes);

  if (command === SPI_REPLY) {
    for (var i = 5, len = board.currentBuffer.length; i < len - 1; i += 2) {
      reply.push((board.currentBuffer[i + 1] << 7) | board.currentBuffer[i]);
    }
    board.emit("spi-data-" + deviceId + "-" + channel, reply);
  }
};
...
function Board(port, options, callback) {
  ...
  this.SPI_CS_ACTIVE_STATE = {
    LOW: 0x00,
    HIGH: 0x01
  };

  // store 1 deviceId per SPI channel
  this.spiDeviceIds = [];
  this.spiCsPins = [];
...
Board.prototype.spiConfig = function(channel) {
...
Board.prototype.spiBeginTransaction = function(deviceId, options) {
  var channel = 0;
  var bitOrder;
  var dataMode;
  var maxClockSpeed;
  var csPin;
  var csActiveState;
  var wordSize = 8; // number of bits in a word

  if (typeof options === "object" && options !== null) {
    channel = options.channel || 0;
    bitOrder = options.bitOrder;
    dataMode = options.dataMode;
    maxClockSpeed = options.maxClockSpeed;
    csPin = options.csPin;
    csActiveState = options.csActiveState;
  }

  if (typeof deviceId === "undefined") {
    throw new Error("Must specify a deviceId when calling spiBeginTransaction.");
  }

  if (deviceId > 31) {
    throw new Error("A SPI deviceId value must be between 0 and 31.");
  }

  this.spiDeviceIds[channel] = deviceId;
  ...
  var data = [
    ...
    deviceId << 2 | channel,
    dataMode << 1 | bitOrder,
    maxClockSpeed & 0x0000007F,
    (maxClockSpeed >> 7) & 0x0000007F,
    (maxClockSpeed >> 14) & 0x0000007F,
    (maxClockSpeed >> 21) & 0x0000007F,
    (maxClockSpeed >> 28) & 0x0000007F,
    wordSize
  ];
  if (typeof csPin !== "undefined") {
    data.push(csPin)
    this.spiCsPins[channel] = {};
    this.spiCsPins[channel].pin = csPin;
    if (typeof csActiveState === "undefined") {
      csActiveState = this.SPI_CS_ACTIVE_STATE.LOW;
    }
    if (csActiveState !== this.SPI_CS_ACTIVE_STATE.LOW &&
        csActiveState !== this.SPI_CS_ACTIVE_STATE.HIGH) {
      // if invalid value, use default active state
      csActiveState = this.this.SPI_CS_ACTIVE_STATE.LOW;
    }
    this.spiCsPins[channel].activeState = csActiveState;
    data.push(csActiveState);
  }
  data.push(END_SYSEX);
  this.transport.write(new Buffer(data));
};

Board.prototype.spiEndTransaction = function(channel) {
  if (typeof channel === "undefined") {
    // default to channel 0
    channel = 0;
  }
...
Board.prototype.spiTransfer = function(options, callback) {
  var channel = 0;
  var numBytes;
  var inBytes;
  var transferOptions;
  ...
  if (typeof options === "object" && options !== null) {
    channel = options.channel || 0;
    inBytes = options.inBytes;
    numBytes = options.numBytes;
    pinOptions = options.pinOptions || 0;
    transferOptions = options.transferOptions || this.SPI_TRANSFER_MODES.READ_WRITE;
  }

  if (typeof this.spiDeviceIds[channel] === "undefined") {
    throw new Error("Must call spiBeginTransaction at least once before calling spiTransfer.");
  }

  if (transferOptions === this.SPI_TRANSFER_MODES.READ_ONLY && numBytes === "undefined") {
    throw new Error("Must specify numBytes when using spiTransfer in read-only mode.");
  }

  if (pinOptions > 0 && typeof this.spiCsPins[channel].pin === "undefined") {
    console.warn("A SPI chip select pin was not defined but chip select pin options were set.");
    pinOptions = 0x01; // disable CS pin
  }

  if (inBytes && typeof numBytes === "undefined") {
    numBytes = inBytes.length
  }
  ...
  data.push(channel);
  data.push(pinOptions << 2 | transferOptions);
  data.push(numBytes);
  if (inBytes) {
    for (var i = 0, len = inBytes.length; i < len; i++) {
      data.push(inBytes[i] & 0x007F);
...
Board.prototype.spiWrite = function(inBytes, pinOptions) {
  if (typeof inBytes === "undefined" || inBytes.length < 1) {
    throw new Error("Must an array of 1 or more bytes to spiWrite.");
  }
  ...
  this.spiTransfer({
    ...
    transferOptions: this.SPI_TRANSFER_MODES.WRITE_ONLY
  });
};

Board.prototype.spiRead = function(numBytesToRead, pinOptions, callback) {
  if (typeof numBytesToRead === "undefined" || numBytesToRead < 1) {
    throw new Error("Must pass the number of bytes to read to spiRead.");
  }
  if (arguments.length === 2 && typeof pinOptions === "function") {
    callback = pinOptions;
    pinOptions = 0;
  }
  this.spiTransfer({
    ...
    pinOptions: pinOptions,
    transferOptions: this.SPI_TRANSFER_MODES.READ_ONLY
  }, callback);
};

Board.prototype.spiReadWrite = function(inBytes, pinOptions, callback) {
  if (typeof inBytes === "undefined" || inBytes.length < 1) {
    throw new Error("Must an array of 1 or more bytes to spiReadWrite.");
  }
  if (arguments.length === 2 && typeof pinOptions === "function") {
    callback = pinOptions;
    pinOptions = 0;
  ...
  this.spiTransfer({
    ...
    numBytes: inBytes.length,
    pinOptions: pinOptions,
    transferOptions: this.SPI_TRANSFER_MODES.READ_WRITE
  }, callback);
};

Board.prototype.spiEnd = function(channel) {
  if (typeof channel === "undefined") {
    // default to channel 0
    channel = 0;
  }
